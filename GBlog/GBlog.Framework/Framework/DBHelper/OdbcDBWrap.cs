...
{
    /// <summary>
    /// COdbcDBWrap 使用ADO.NET中ODBC的一套对象操作数据库
    /// </summary>
    public class COdbcDBWrap : IDBInterface
    {
        ...
        /// 数据库链接对象
        /// </summary>
        private OdbcConnection m_objConn = null;
        /// <summary>
        /// 事务对象
        /// </summary>
        private OdbcTransaction m_objTrans = null;
        ...
        public COdbcDBWrap(string strConnectString)
        {
            // 系统参数替换
            m_strConnectString = strConnectString;
            m_bTrans = false;
            m_objConn = new OdbcConnection(m_strConnectString);
        }
        /// <summary>
        /// 析构
        /// </summary>
        ...
        /// <param name="bCloseDB">是否关数据库</param>
        /// <param name="strError">异常信息</param>
        /// <returns>最大ID值</returns>
        ...
        public IDataReader GetDataReaderBySQL(string strSQL, out string strError)
        {
            ...
            try
            {
                using (OdbcCommand objCommand = new OdbcCommand())
                {
                    objCommand.Connection = m_objConn;
                    objCommand.CommandText = strSQL;
                    objCommand.CommandType = CommandType.Text;
        ...
        public DataSet GetDataSetByProcedure(string strProcedureName, bool bCloseDB, out string strError)
        {
            ...
            //// oracle数据库通过sql得到数据集
            //if (this.m_strConnectString.ToLower().IndexOf("oraOdbc") != -1 || this.m_strConnectString.ToLower().IndexOf("msdaora") != -1)
            //{
        ...
        private OdbcParameter CheckParameter(OdbcParameter parameter)
        {
