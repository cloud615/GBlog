...
{
    /// <summary>
    /// COracleDBWrap 使用ADO.NET中Oracle的一套对象操作数据库
    /// </summary>
    public class COracleDBWrap : IDBInterface
    {
        ...
        /// 数据库链接对象
        /// </summary>
        private OracleConnection m_objConn = null;
        /// <summary>
        /// 事务对象
        /// </summary>
        private OracleTransaction m_objTrans = null;
        /// <summary>
        /// 构造函数
        /// </summary>
        public COracleDBWrap()
        {
        }
        ...
        public COracleDBWrap(string strConnectString)
        {
            m_strConnectString = strConnectString;
            m_bTrans = false;
            m_objConn = new OracleConnection(m_strConnectString);
        }
        /// <summary>
        /// 析构
        /// </summary>
        ...
        public bool OpenDB(out string strError)
        {
            ...
            try
            {
                ...
                if (m_objConn.State == System.Data.ConnectionState.Closed)
                {
                    m_objConn.Open();
                }
                bResult = true;
        ...
        /// </summary>
        /// <param name="strError">返回关闭连接失败原因</param>
        /// <returns>布尔值表示成功或失败</returns>
        ...
        public bool IsOpen()
        {
            ...
            try
            {
                ...
                {
                    bResult = !(m_objConn.State == System.Data.ConnectionState.Closed);
                }
            }
            catch (Exception ex)
            {
        ...
        public int GetMaxID(string strTableName, out string strError)
        {
            ...
            DataSet ds = GetDataSetBySQL(strSQL, true, out strError);
        ...
        /// <param name="listParam">参数集合</param>
        /// <returns>返回第一行第一列的结果</returns>
        private T GetScalarByProcedure<T>(string strProcedureName, out string strError, params IDbDataParameter[] listParam)
        {
            ...
            try
            {
                ...
                {
                    foreach (OracleParameter parameter in listParam)
                    {
        ...
        public DataSet GetDataSetBySQL(string strSQL, bool bCloseDB, out string strError)
        {
            ...
            if (strError != "")
            {
                // 解决在linux上oracle存在不稳定错误
                objDataSet = GetDataSetBySQL(strSQL, out strError);
            }
            if (bCloseDB)
            {
                this.CloseDB(out strCloseError);
        ...
        public DataSet GetDataSetBySQLPos(string strSQL, int startRecord, int maxRecords, string srcTable, bool bCloseDB, out string strError)
        {
            ...
            {
                // 解决在linux上oracle存在不稳定错误
                objDataSet = GetDataSetBySQLPos(strSQL, startRecord, maxRecords, srcTable, out strError);
            }
        ...
        {
            DataSet objDataSet = GetDataSetByProcedure(strProcedureName, out strError);
            if (strError != "")
            {
                // 解决在linux上oracle存在不稳定错误
        ...
        /// <summary>
        /// oledb字符串转oracle连接字符串
        /// </summary>
        /// <param name="strConn">oledb的字符串</param>
        /// <returns></returns>
        public static string TransConn(string strConn)
        {
            ...
            {
                if (item.ToLower().IndexOf("data source") != -1)
                {
                    strDs = item.Split('=')[1].Trim();
                }
                else if (item.ToLower().IndexOf("user id") != -1)
                {
                    Uid = item.Split('=')[1].Trim();
                }
                else if (item.ToLower().IndexOf("password") != -1)
                {
                    Pws = item.Split('=')[1].Trim();
                }
            }
            strRet = string.Format("Data Source={0};User Id={1};Password={2};", strDs, Uid, Pws);
        ...
        private OracleParameter CheckParameter(OracleParameter parameter)
        {
